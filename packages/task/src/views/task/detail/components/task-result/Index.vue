...
<template>
  <div class="result-wrapper">
    <div class="result-title">
      ...
      <hl-icon :icon="reverse ? 'bx:sort-up' : 'bx:sort-down'" size="20px" class="cursor-pointer" @click="reverse = !reverse" />
    </div>
    <div class="result-content pr-1">
      <el-timeline v-if="result.length > 0" style="max-width: 600px">
        <result-item v-for="item in result_comp" :key="item.id" :data="item" :reverse="reverse" @remove="handleRemove" @refresh="handleRefresh" />
      </el-timeline>
