...
<template>
  <el-dropdown class="hl-dropdown" trigger="click" @visible-change="val => { arrowStatus = val }">
    ...
    <template #dropdown>
      <div class="flex flex-col hl-dropdown-popover">
        ...
        <el-scrollbar class="py-1" max-height="280">
          <div v-if="multiple" class="pr-4">
            ...
            <el-checkbox-group :model-value="_value" class="flex flex-col">
              <el-checkbox v-for="item in _options" :key="item.value" :disabled="item.disabled" :label="item.label" :value="item.value" class="mr-0 ml-4" @click="handleSingleChange(item)" />
            </el-checkbox-group>
          </div>
          <template v-else>
            <el-dropdown-menu>
